...
Optional:
    ...
    python aurion_capture_python.py --choice-capture
...
def get_current_page_kind(driver: webdriver.Chrome) -> str:
    try:
        url = str(driver.current_url or '')
    except Exception:
        return 'unknown'

    if 'ChoixPlanning.xhtml' in url:
        return 'choice'
    if 'Planning.xhtml' in url:
        return 'planning'
    return 'unknown'
...
def emit_capture_heartbeat(driver: webdriver.Chrome, current_id: str) -> None:
    page_kind = get_current_page_kind(driver)
    planning_label = read_planning_label(driver) or 'Planning'
    print(f"[heartbeat] page={page_kind} planning={planning_label} | idInit={current_id}", flush=True)
...
def capture_snapshot(driver: webdriver.Chrome, target_label: str) -> Dict[str, object]:
    ...
    return {
        ...
        'pageKind': get_current_page_kind(driver),
            ...
            if time.monotonic() >= next_heartbeat:
                if 'Planning.xhtml' in driver.current_url:
                    ...
                    if current_id.startswith('webscolaapp.Planning_'):
                        emit_capture_heartbeat(driver, current_id)
                next_heartbeat = time.monotonic() + 5.0
...
def run_choice_capture(driver: webdriver.Chrome, output_path: Path) -> List[Dict[str, object]]:
    existing_entries = load_existing_entries(output_path)
    seen_ids = {str(entry.get('idInit', '')) for entry in existing_entries if isinstance(entry, dict)}
    entries: List[Dict[str, object]] = list(existing_entries)
    ...
    driver.get(CHOIX_URL)
    wait_for_choice_page(driver)
    print('Choice capture mode.')
    print('Click planning choices on the ChoixPlanning page; the script will save each new choice idInit automatically.')
    print('Press Ctrl+C to stop.')
    ...
    while True:
        try:
            ...
            if time.monotonic() >= next_heartbeat:
                if get_current_page_kind(driver) == 'choice':
                    current_id = read_id_init(driver)
                    if current_id.startswith('webscolaapp.ChoixPlanning_'):
                        emit_capture_heartbeat(driver, current_id)
                next_heartbeat = time.monotonic() + 5.0
            ...
            if not current_id or not current_id.startswith('webscolaapp.ChoixPlanning_'):
                continue
            ...
            seen_ids.add(snapshot_id)
            save_output(output_path, entries, driver.current_url)
            print(f"  - captured choice idInit={snapshot_id}, codes={snapshot.get('codesCount', 0)}")

            last_seen_id = current_id
            last_seen_body = current_body
        except KeyboardInterrupt:
            print('\nChoice capture stopped.')
            break
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--choice-capture', action='store_true', help='Capture ids while you choose plannings on the ChoixPlanning page yourself')
...
def main() -> int:
    ...
    try:
        ...
        elif args.choice_capture:
            entries = run_choice_capture(driver, output_path)
        else:
